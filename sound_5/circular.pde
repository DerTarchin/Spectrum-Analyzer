public class Circular extends View {
  ...
  public void resetDefaults() {
    ...
    rms_boost_vol_min = 0.26;
    rms_boost_change_min = 10;
  ...
  public void draw() {
    ...
    if(diff>rms_boost_change_min && rms_high>rms_boost_vol_min) {
      //println(rms_high);
      // map boost level to how powerful the beat is
      //float boost = map(rms_high, rms_boost_vol_min, 0.5, 0, (rms_boost_vol_min + rms_boost_val_max)/2);
      float boost = map(diff, 0, 100, 0, rms_boost_val_max);
      rms_boost_val = rms_boost_val + boost;
      //println(rms_boost_val);
      println(int(diff) + " : " + rms_boost_val);
    }
  ...
  private void drawBg() {
    ...
    //blur.noStroke();
    //blur.fill(255);
    //blur.rect(10,10,50,rms_avg*300);
    //println(rms_avg*300);
  }
  ...
  private float getValue(int i){
    float r = diameter/2;
    if(transition > 0) avg[0] *= 0.66;
    float val = 0;      
    if(i < bands-transition) val = avg[i]*amp;
    else if(i >= bands-transition) {
      val = this.diff(avg[0], avg[bands-1-transition], 
      1/float(transition)*(transition-(bands-i)))*amp;
    }
    val = constrain(val, min(r, 0), max(r, 0));
    return constrain(val, min(r, val+minheight), max(r, val+minheight));
  }
